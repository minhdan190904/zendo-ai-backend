...
await y
    .scriptName("ImageFX")
    .command("generate", "Generate new images", (yargs) => {
    return yargs
        .option("prompt", {
        alias: "p",
    ...
    const prompt = new Prompt({
        ...
        generationModel: argv.model,
        aspectRatio: ("IMAGE_ASPECT_RATIO_" + argv.size),
    });
    ...
    .command("caption", "Generate detailed caption(s) from image", (yargs) => {
    return yargs
        .option("image", {
        alias: "i",
    ...
    const generatedCaptions = await fx.generateCaptionsFromImage(argv.image, ImageType[argv.type], // Safe typecast ;)
    argv.count);
    ...
    .command("fetch <mediaId>", "Download a generated image with its mediaId", (yargs) => {
    return yargs
        .positional("mediaId", {
