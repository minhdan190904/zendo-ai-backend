...
export class ImageFX {
     ...
     */
    async generateImage(prompt, retries = 0) {
        ...
        return generatedImages.map((data) => new Image(data));
    }
    /**
    ...
    async getImageFromId(id) {
        ...
        const params = encodeURIComponent(JSON.stringify({ json: { mediaKey: id } }));
        try {
     ...
     */
    async generateCaptionsFromImage(imagePath, imageType, count = 1) {
        ...
        try {
            ...
            const parsedResponse = await response.json();
            const imageCaption = parsedResponse?.result?.data?.json?.result?.candidates;
